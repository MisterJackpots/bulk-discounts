  ...
  describe 'model methods' do
    ...
    describe "#incomplete_invoices" do
      ...
      before(:each) do
        ...
        @discount1 = BulkDiscount.create!(percentage: 20, quantity_threshold: 800, merchant_id: @merchant_3.id)
        ...
        @invoice_item_7 = InvoiceItem.create!(item_id: @item_6.id, invoice_id: @invoice_7.id, quantity: 801, unit_price: 90999, status: 2)
      end
      ...
      describe "#total_revenue" do
        it "should return the total revenue that will be generated by the specified invoice" do
          expect(@invoice_4.total_revenue).to eq(3169708)
          expect(@invoice_5.total_revenue).to eq(12005515)
          expect(@invoice_6.total_revenue).to eq(67157262)
        end 
      end

      describe '#discounted_revenue' do
        it 'returns the discounted revenue for this invoice, if applicable' do
          expect(@invoice_6.discounted_revenue).to eq(67157262)
          expect(@invoice_7.discounted_revenue).to eq(58312159)
        end
      end
    end 
  end 
